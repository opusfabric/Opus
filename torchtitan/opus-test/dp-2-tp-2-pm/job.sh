...
#SBATCH --account=m4999 # replace with your allocation
...
# --------------------------
CONFIG_FILE=${CONFIG_FILE:-"~/Opus/torchtitan/opus-test/dp-2-tp-2-pm/llama3_debug.toml"}
...
if [ \"\$NODE_HOSTNAME\" = \"$(echo $SERVER_IPS | cut -d',' -f1)\" ]; then
    ...
    ./controller -d 0 -e -r 4 > ~/Opus/torchtitan/opus-test/dp-2-tp-2-pm/controller.ans 2>&1 &
fi
...
torchrun --nnodes=$NUM_NODES --nproc_per_node=$NUM_RANKS_PER_NODE \
        ...
        # > ./opus-test/dp-2-tp-2-pm/output_node_\$SLURM_PROCID.ans 2>&1
"
