...
class Trainer(torch.distributed.checkpoint.stateful.Stateful):
    ...
    def __init__(self, job_config: JobConfig):
        ...
        # --- FIX START: Patch for DTensor + Pipeline Parallelism compatibility ---
        try:
            import torch.distributed.pipelining.stage as stage
            def fixed_scale_grads(self, grad_scale_factor):
                for p in self.submod.parameters():
                    if p.grad is not None:
                        # Use out-of-place division to support DTensor redistribution
                        p.grad = p.grad / grad_scale_factor
            
            stage.PipelineStage.scale_grads = fixed_scale_grads
            logger.info("Applied monkey patch to PipelineStage.scale_grads for DTensor compatibility")
        except (ImportError, AttributeError) as e:
            logger.warning(f"Failed to patch PipelineStage: {e}")
        # --- FIX END ---
        ...
        print(f"Hostname: {os.uname().nodename}, LOCAL_RANK: {os.environ['LOCAL_RANK']}")
        ...
        dist_utils.init_distributed(
            ...
            enable_opus=job_config.training.enable_opus_backend,
            base_folder=job_config.job.dump_folder,
        )

        logger.info("!!!!!default process group initialized!!!!!")
        ...
        logger.info("!!!!!all process groups initialized!!!!!")
